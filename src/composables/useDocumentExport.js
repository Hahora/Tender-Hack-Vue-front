/**
 * Composable: формирование и сохранение итогового документа
 * Генерирует обоснование НМЦ в виде текста и предоставляет
 * возможность скачать его как .txt-файл
 */
import { ref } from 'vue'
...
export function useDocumentExport() {
   ...
   */
  function buildDocument({ steQuery, requestedQty, unit, justificationText, validProcurements, totalNmts }) {
    const now = new Date()
  ...
  function saveDocument(params) {
    ...
    const safeName = params.steQuery.replace(/[^а-яА-Яa-zA-Z0-9]/g, '_').slice(0, 40)
