...
export function usePriceCalculation(procurements, filters, requestedQty) {
  /**
   * Шаг 1: Все фильтры применяются на сервере — возвращаем данные как есть
   */
  const filteredProcurements = computed(() => procurements.value);
  ...
  const validProcurements = computed(() =>
    processedProcurements.value.filter(
      (p) => (!p.isOutlier || p.manualInclude) && !p.isExcluded
    )
  );
